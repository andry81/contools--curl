...
if /i "%CONTOOLS_CURL_PROJECT_ROOT_INIT0_DIR%" == "%~dp0" exit /b 0

set "CONTOOLS_CURL_PROJECT_ROOT_INIT0_DIR=%~dp0"
...
call "%%~dp0canonical_path_if_ndef.bat" CONTOOLS_CURL_PROJECT_ROOT                "%%~dp0.."
...
call "%%~dp0canonical_path_if_ndef.bat" PROJECT_OUTPUT_ROOT                       "%%CONTOOLS_CURL_PROJECT_ROOT%%/_out"
call "%%~dp0canonical_path_if_ndef.bat" PROJECT_LOG_ROOT                          "%%CONTOOLS_CURL_PROJECT_ROOT%%/.log"

call "%%~dp0canonical_path_if_ndef.bat" CONTOOLS_CURL_PROJECT_INPUT_CONFIG_ROOT   "%%CONTOOLS_CURL_PROJECT_ROOT%%/_config"
call "%%~dp0canonical_path_if_ndef.bat" CONTOOLS_CURL_PROJECT_OUTPUT_CONFIG_ROOT  "%%PROJECT_OUTPUT_ROOT%%/config/contools--curl"

rem retarget externals of an external project

call "%%~dp0canonical_path_if_ndef.bat" CONTOOLS_PROJECT_EXTERNALS_ROOT           "%%CONTOOLS_CURL_PROJECT_EXTERNALS_ROOT%%"

rem init immediate external projects

if exist "%CONTOOLS_CURL_PROJECT_EXTERNALS_ROOT%/contools/__init__/__init__.bat" (
  rem disable code page change in nested __init__
  set /A NO_CHCP+=1
  call "%%CONTOOLS_CURL_PROJECT_EXTERNALS_ROOT%%/contools/__init__/__init__.bat" -no_load_user_config || exit /b
  set /A NO_CHCP-=1
)

if %NO_GEN%0 EQU 0 (
  call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/mkdir_if_notexist.bat" "%%CONTOOLS_CURL_PROJECT_OUTPUT_CONFIG_ROOT%%" || exit /b
)
